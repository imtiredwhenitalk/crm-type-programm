...
from db.workersdb import (
    DEPARTMENTS, db_get_workers, db_delete_workers, db_add_worker,
    db_update_worker, db_get_departments, db_worker_stats,
)
...
def _workers_list(self, parent):
    ...
    dept_f = _user_dept_filter()
    if dept_f:
        tk.Label(right_f, text=f"Відділ: {dept_f}",
...
def worker_reload(self):
    ...
    dept = _user_dept_filter()
    rows = db_get_workers(q, dept_filter=dept or '', status_filter=st)
...
def _worker_form(self, existing):
    ...
    dept_vals = list(dict.fromkeys(dept_vals))
...
def _workers_stats(self, parent):
    ...
    def on_select(event):
        ...
        if vals and vals[0] != "ВСЬОГО":
            dept = vals[0]
            workers = db_get_workers(dept_filter=dept if dept != "(без відділу)" else '')
            names = ", ".join(w[1] for w in workers[:10])
            extra = f" (+{len(workers)-10} ще)" if len(workers) > 10 else ""
            messagebox.showinfo(f"Відділ: {dept}",
                                f"Працівників: {vals[1]}\n\n{names}{extra}")
